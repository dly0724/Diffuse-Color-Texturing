...
void CChildView::InitGL()
{
	...
	// Task 2
	m_toriTex.LoadFile(L"textures/marble02.bmp");
	m_tori.m_program = LoadShaders("ShaderWnd/vertexSphere.glsl", "ShaderWnd/fragmentSphere.glsl");
	m_tori.InitGL();
}
...
void CChildView::RenderGL()
{
	...
	M = translate(mat4(1.f), vec3(0., 0., 10.)) * rotate(mat4(1.f), -90.f, vec3(1., 0., 0.));

	glUniformMatrix4fv(m_nPVM, 1, GL_FALSE, value_ptr(m_mPVM * M));
	glUniformMatrix4fv(m_nVM, 1, GL_FALSE, value_ptr(m_mVM * M));
	...
	glUniformMatrix4fv(m_nVM, 1, GL_FALSE, value_ptr(VM));
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, m_toriTex.TexName());
